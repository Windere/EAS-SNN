...
class SpikingYOLOXHead(nn.Module):
    def __init__(
            self,
            num_classes,
            width=1.0,
    ...
    def initialize_biases(self, prior_prob):
        for module in self.cls_preds.modules():
            if isinstance(module, nn.Conv2d):
                b = module.bias.view(1, -1)
                b.data.fill_(-math.log((1 - prior_prob) / prior_prob))
                module.bias = torch.nn.Parameter(b.view(-1), requires_grad=True)

        for module in self.obj_preds.modules():
            if isinstance(module, nn.Conv2d):
                b = module.bias.view(1, -1)
                b.data.fill_(-math.log((1 - prior_prob) / prior_prob))
                module.bias = torch.nn.Parameter(b.view(-1), requires_grad=True)
