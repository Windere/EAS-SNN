...
from yolox.utils.event_reps import timesurface_measure, count_measure, to_voxel_grid_numpy,to_voxel_cube_numpy,to_timesurface_numpy
...
class GEN1Dataset(torchDataset):
    def __init__(self, data_path, input_size, random_aug=True, img_size=(240, 304), continuous=True,
                 slice_policy='fix_t',
                 cache_path=None, letterbox_image=True, map_val=False, format='cxcywh',
                 target_transform=None, class_names=GEN1_CLASSES,
                 **slice_args):  # todo: consider if merge slice_policy, num_slice, slice_args, continuous
        """
        :param data_path: path to the train/val/test part of gen1 dataset
        ...
        """
        super(GEN1Dataset, self).__init__()
    ...
    def __getitem__(self, item):
        ...
        frames = self.generate_slices(file, time, self.slice_args['num_slice'], self.continuous)
        t2 = lib_time.time()
        # logger.info('{}-{}'.format(frames.min(), frames.max()))
    ...
    def extract_labels(self, label_pathes, type='.npy'):
        ...
        for label_path in label_pathes:
            for file in (os.listdir(label_path)):
                if file.endswith(type) and re.split("_bbox|_td", file)[0] not in dirs_to_ignore['gen1']:
                    files += [os.path.join(label_path, file)]
                elif file.endswith(type) and re.split("_bbox|_td", file)[0] in dirs_to_ignore['gen1']:
    ...
    def agrregate(self, events, method):
        ...
        if method == 'sum':
            ...
            slices,_ = self.slice_events(events, self.slice_args['micro_slice'])
            ...
            slices,dt = self.slice_events(events, self.slice_args['micro_slice'])
            frame = to_timesurface_numpy(slices, sensor_size=[self.img_size[1], self.img_size[0], 2],dt=dt,tau=50e3)

        elif method == 'voxel_cube':
            frame = to_voxel_cube_numpy(events, sensor_size=[self.img_size[1], self.img_size[0], 2],num_slices=self.slice_args['micro_slice'])
