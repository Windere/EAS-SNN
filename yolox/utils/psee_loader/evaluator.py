...
from yolox.utils.psee_loader.evaluation import evaluate_list


# this class is from https://github.com/uzh-rpg/RVT/tree/master
...
class PropheseeEvaluator:
    ...
    def evaluate_buffer(self, img_height: int, img_width: int) -> Optional[Dict[str, Any]]:
        ...
        logger.info(f"Starting evaluation with {len(predictions)} predictions and {len(labels)} labels.")
        logger.info(
            f"Image size: {img_height}x{img_width} for dataset {self.dataset} with downsample_by_2={self.downsample_by_2}")
