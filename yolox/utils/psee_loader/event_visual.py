...
def play_event_stream(events, delta_t, img_size=(240, 304)):
    start_id = 0
    start_t = events[start_id]['t']
    height, width = img_size
    im = np.zeros((height, width, 3))
    for i in range(len(events)):
        if events[i]['t'] > start_t + delta_t:
            im = vis.make_binary_histo(events[start_id:i], img=im, width=width, height=height)
            cv2.imshow('out', im)
            cv2.waitKey(1)
            start_id = i
            start_t = events[start_id]['t']
...
def play_files_parallel(td_files, labels=None, delta_t=50000, skip=0, save_path=None):
    """
...
def parse_args():
    ...
    parser.add_argument('-t', '--to', default='visulize.mp4', type=str, help="save path")
...
if __name__ == '__main__':
    ARGS = parse_args()
    play_files_parallel(ARGS.records, skip=ARGS.skip, delta_t=ARGS.delta_t, save_path=ARGS.to)
